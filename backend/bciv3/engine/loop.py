...
def design(topic_id: str, prompt: str = "", *, rounds: int = 2, lens: str = "biomimicry",
           backend: str = "auto", model: str | None = None, constraint: str | None = None) -> dict:
    """Invent → simulate → (on fail) refine, up to `rounds`. Returns the best candidate + history."""
    inv = get(topic_id)
    cand = invent(topic_id, prompt, lens=lens, backend=backend, model=model, constraint=constraint)
...
def rank(topic_id: str, prompt: str = "", *, lenses: list[str] | None = None,
         backend: str = "auto", model: str | None = None, constraint: str | None = None) -> list[dict]:
    """Generate one candidate per lens, grade them, and return them best-first."""
    lenses = lenses or list(LENSES)
    out = []
    for ln in lenses:
        c = invent(topic_id, prompt, lens=ln, backend=backend, model=model, constraint=constraint)
